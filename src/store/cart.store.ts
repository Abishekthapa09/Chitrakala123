...
export type PromoCodes = 'ILOVEPAINTING100' | 'ILOVEPAINTING50' | '';
type CartState = {
  cart: TCartItem[];
...
export const useCartStore = create<InitCartStore>((set, get) => ({
  ...
  setCartDiscount: code => {
    if (code === 'ILOVEPAINTING100') set({ cartDiscount: 'full-disc' });
    if (code === 'ILOVEPAINTING50') set({ cartDiscount: 'half-disc' });
