...
const buttonVariants = cva(
  ' block text-2xl text-Light_grayish_blue text-center transition-all outline-none outline-offset-0 focus:outline-none focus-visible:outline-offset-0 focus-visible:outline-1 focus-visible:outline-Dark_grayish_blue',
  {
    variants: {
      variant: {
        cart_chk:
          'overflow-hidden bg-Primary px-3 py-6 rounded-xl font-bold capitalize active:translate-y-px max-w-xl mx-auto xl:text-3xl',
        input_btn:
          'bg-Primary focus-visible:outline focus-visible:outline-2 focus-visible:outline-Light_grayish_blue px-8 py-4 overflow-hidden rounded-tr-xl rounded-br-xl font-bold transition-transform',
        ...
        hero_main:
          'overflow-hidden font-orbitron tracking-wider ring-2 ring-Light_grayish_blue px-6 py-10 capitalize text-3xl rounded-lg hover:ring-Primary focus-visible:bg-Primary focus-visible:ring-0',
        category:
        ...
        navigation:
          'bg-Grayish_blue/50 p-2 rounded-xl flex-center fill-Very_dark_blue hover:fill-Primary focus-visible:fill-Primary focus-visible:outline-1 focus-visible:outline-Very_dark_blue',
        profile:
        ...
        profile_del:
          'scale-75 rounded-md bg-Primary/25 p-3 hover:ring-1 hover:ring-Dark_grayish_blue',
        caruosel:
  ...
  ({ href = '', hasRipple, rippleColor, variant, size, className, ...props }, ref) => {
    ...
    const ripElements = ripples.map((ripple, idx) => (
      <span
        key={idx}
        className={`absolute ${
          rippleColor ? rippleColor : 'bg-Pale_Primary'
