...
export const PromoCodeInput = () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const fullDisc: PromoCodes = 'ILOVEPAINTING100';
    const halfDisc: PromoCodes = 'ILOVEPAINTING50';
    ...
    <form
      ...
      <label htmlFor="promo-code" className="peer flex-1">
        <input
          ...
          className="w-full bg-Light_grayish_blue text-2xl text-Dark_grayish_blue rounded-tl-xl rounded-bl-xl py-4 px-8 outline-none placeholder:text-Grayish_blue placeholder:text-xl focus-visible:outline focus-visible:outline-offset-0 focus-visible:outline-2 focus-visible:outline-Primary"
        />
      </label>
      <Button
        hasRipple
      ...
      <p
        className={`absolute top-20 left-1/2 -translate-x-1/2 text-xl text-center w-full font-bold tracking-wide transition-all ${
          promoCode.status === 'valid' ? 'text-green-400' : 'text-Primary'
        } ${
          promoCode.message
