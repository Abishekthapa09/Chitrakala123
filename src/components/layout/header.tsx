...
import profileImg from '../../images/Avtar.png';
import { MenuIcon, CartIcon, UserIcon } from '../icons';
import logo from '../../images/Chitrakala_Logo.png';
...
export const Header = ({ children }: PropsWithChildren) => {
  ...
  return (
    <header className="sticky top-0 z-10 flex items-center justify-between px-8 py-8 bg-Overlay">
      ...
      <SearchSection/>
        ...
        <Button ref={cartRef} title="view cart" className="relative w-10 h-10 flex-center">
          {cart.length > 0 && (
            <span className="absolute -top-3 left-3 bg-Primary px-2 min-w-[2.25rem] text-White text-lg rounded-xl flex-center font-bold pointer-events-none">
              {totalQty}
            </span>
          )}
        ...
        <Button
          ...
          className="hover:ring-2 hover:ring-Primary rounded-full focus-visible:outline-2 focus-visible:outline-Primary">
          {user ? (
            <img
